...
public class LegacyAnt {
  ...
  public LegacyAnt(int solutionLength, int numberOfRows, int numberOfColumns) {
    ...
    if (!ProblemConfiguration.ALLOW_VISITED_PIXELS) {
      for (ImagePixel visitedPixel : pixelPath) {
        if (visitedPixel != null
            && visitedPixel.getxCoordinate() == imagePixel.getxCoordinate()
            && visitedPixel.getyCoordinate() == imagePixel.getyCoordinate()
  ...
  private List<PosiblePixel> getProbabilities(double[][] pheromoneTrails,
    ...
    List<ImagePixel> neighbours = currentPosition.getNeighbourhood(imageGraph);
    ...
    for (ImagePixel neighbour : neighbours) {
      if (!isPixelVisited(neighbour)
        ...
        double heuristicValue = 1 / Math.abs(neighbour.getGreyScaleValue()
            - getMeanGrayScaleValue() + ProblemConfiguration.DELTA);

        double pheromoneTrailValue = pheromoneTrails[neighbour.getxCoordinate()][neighbour
            .getyCoordinate()] + ProblemConfiguration.DELTA;
